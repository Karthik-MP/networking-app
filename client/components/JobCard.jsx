...
export default function JobCard({ job, onPress }) {
  ...
  return (
    <Pressable onPress={onPress} className="bg-white rounded-2xl p-4 mb-3 border border-gray-200">
      <Text className="text-base font-semibold" numberOfLines={1}>{title}</Text>
      <Text className="text-gray-700 mt-1" numberOfLines={1}>{company}</Text>
      <View className="flex-row items-center mt-2">
        <Ionicons name="location-outline" size={16} color="#6b7280" />
        <Text className="text-gray-600 ml-1 flex-1" numberOfLines={1}>{locations || '—'}</Text>
      </View>
      {!!salary && (
